  ...
  try {
    ...
    if (totalPrice < discount.min_order_value) {
      return res.status(400).json({
        message: `Đơn hàng tối thiểu ${discount.min_order_value.toLocaleString(
          "vi-VN"
...
exports.completeCheckout = async (req, res) => {
  try {
    ...
    if (discountCode) {
      ...
      if (discountData) {
        ...
        if (totalPrice < discountData.min_order_value) {
          return res.status(400).json({
            message: `Đơn hàng tối thiểu ${discountData.min_order_value.toLocaleString(
              "vi-VN"
            )} VNĐ để áp dụng mã giảm giá này`,
          });
        }
    ...
    // Import socketManager để thiết lập timeout tự động hủy đơn hàng
    const { setOrderPaymentTimeout } = require("../socketManager");
    ...
    if (paymentMethod === "cash") {
      const newOrder = new Order({
        ...
        status: "Đang xử lý",
        items: orderItems,
        // Không cần timeout cho COD vì thanh toán khi nhận hàng
      });
    ...
    if (
      ...
      // Tạo thời gian hết hạn thanh toán (60 giây từ bây giờ)
      const paymentTimeout = new Date(Date.now() + 60000);
      ...
      const newOrder = new Order({
        ...
        status: "Đang xử lý",
        items: orderItems,
        paymentTimeout: paymentTimeout, // Thêm thời gian hết hạn thanh toán
      });

      await newOrder.save();
      console.log("Created new order:", newOrder);

      // Thiết lập tự động hủy đơn hàng sau 60 giây nếu không thanh toán
      setOrderPaymentTimeout(newOrder._id, req.user.id, 60000);
      ...
      if (!paymentResponse.data || !paymentResponse.data.paymentUrl) {
        return res
          .status(500)
          .json({ message: "Không thể tạo thanh toán qua PayOS" });
      }
      ...
      // Tạo thông báo cho người dùng về việc đơn hàng sẽ bị hủy sau 60 giây
      const notification = new Notification({
        user: req.user.id,
        type: "order",
        message: `Đơn hàng #${newOrder.order_code} của bạn sẽ tự động bị hủy sau 60 giây nếu không hoàn tất thanh toán`,
