  ...
  try {
        ...
        if (!categories || categories.length === 0) {
          ...
          const categoryList = categories.map((cat) => ({
            ...
            subCategories: cat.sub_categories
              ? cat.sub_categories.map((sub) => sub.name)
              : [],
          }));
    ...
    // Lấy sản phẩm theo tên
      ...
      const products = await Product.find({
        ...
        .lean()
        .limit(5); // Thêm .lean() để chuyển đổi Mongoose document thành plain JavaScript object
      ...
      if (products.length === 0) {
        ...
        const productList = products.map((p) => {
              ...
              if (
                ...
                const subCategory = category.sub_categories.find(
                  (sub) =>
                    sub._id &&
                    p.brand &&
                    sub._id.toString() === p.brand.toString()
                );
                if (subCategory) {
          ...
          if (p.colors && Array.isArray(p.colors) && p.colors.length > 0) {
            ...
            console.log(
              "Không tìm thấy thông tin màu sắc cho sản phẩm:",
              p.name
            );
          }
          ...
          const productList = products.map((p) => ({
            ...
            colors:
              Array.isArray(p.colors) && p.colors.length > 0
                ? p.colors
                : ["Chưa cập nhật thông tin màu sắc"],
          }));
    ...
    // Lấy sản phẩm theo khoảng giá
      ...
      if (products.length === 0) {
        response = {
          products: [],
          message: `Không tìm thấy sản phẩm nào trong khoảng giá ${minPrice.toLocaleString(
            "vi-VN"
        ...
        response = {
          ...
          message: `Tìm thấy ${
            products.length
          } sản phẩm trong khoảng giá ${minPrice.toLocaleString(
            "vi-VN"
          )}đ - ${maxPrice.toLocaleString("vi-VN")}đ.`,
        };
      }
    ...
    // Lấy sản phẩm có sẵn trong kho
      ...
      if (products.length === 0) {
        ...
        response = {
          ...
          message: `Hiện có tổng cộng ${totalInStock} sản phẩm còn hàng. Đây là 5 sản phẩm tiêu biểu: ${productNames}. Tổng số lượng của 5 sản phẩm này là ${productList.reduce(
            (sum, p) => sum + p.stock,
            0
          )} sản phẩm.`,
        };
...
exports.getGeminiResponse = async (req, res) => {
  ...
  try {
    ...
    const productQueries = [
      ...
      {
        regex: /tìm.*?sản phẩm\s+([\w\s]+)/i,
        query: (match) => `find:${match[1]}`,
      },
      { regex: /tìm\s+([\w\s]+)/i, query: (match) => `find:${match[1]}` },
      {
        regex: /giá.*?từ.*?(\d+).*?đến.*?(\d+)/i,
        query: (match) => `price:${match[1]}-${match[2]}`,
      },
      { regex: /giá.*?([\w\s]+)/i, query: (match) => `find:${match[1]}` },
      {
        regex: /danh mục.*?([\w\s]+)/i,
        query: (match) => `category:${match[1]}`,
      },
      {
        regex: /thương hiệu.*?([\w\s]+)/i,
        query: (match) => `brand:${match[1]}`,
      },
      { regex: /hãng.*?([\w\s]+)/i, query: (match) => `brand:${match[1]}` },
      { regex: /còn hàng|có sẵn/i, query: "instock" },
      // Sửa regex cho truy vấn màu sắc
      {
        regex: /([\w\s]+)\s+có\s+màu\s+(sắc\s+)?gì/i,
        query: (match) => `find:${match[1].trim()}`,
      },
      {
        regex: /màu\s+sắc\s+(của\s+)?([\w\s]+)/i,
        query: (match) => `find:${match[2].trim()}`,
      },
      {
        regex: /màu\s+(của\s+)?([\w\s]+)/i,
        query: (match) => `find:${match[2].trim()}`,
      },
      // Thêm regex cho truy vấn danh mục và thương hiệu
      {
        regex:
          /có\s+những\s+thương\s+hiệu\s+gì|thương\s+hiệu\s+nào|hãng\s+nào/i,
        query: "brands",
      },
      {
        regex:
          /có\s+những\s+sản\s+phẩm\s+gì|sản\s+phẩm\s+nào|danh\s+mục\s+nào|sản\s+phẩm\s+gì/i,
        query: "categories",
      },
    ...
    if (
      message.toLowerCase().match(/([\w\s]+)\s+có\s+màu\s+(sắc\s+)?gì/i) ||
    ...
    for (const { regex, query } of productQueries) {
      ...
      if (match) {
        ...
        try {
          const queryString =
            typeof query === "function" ? query(match) : query;
    ...
    const chat = model.startChat({
      history: [
        ...
        },
        {
          ...
          parts: [
            {
              text: "Hiện tại, GearUp chỉ hoạt động online thông qua website gearup.vn. Chúng tôi không có cửa hàng vật lý, nhưng đảm bảo giao hàng nhanh chóng và an toàn đến tận nơi cho khách hàng trên toàn quốc. Bạn có thể dễ dàng đặt hàng trực tuyến và theo dõi đơn hàng của mình qua website của chúng tôi.",
            },
          ],
        },
      ],
    ...
    if (productInfo) {
      ...
      // Kiểm tra nếu là truy vấn sản phẩm
        ...
        if (productInfo.products.length === 0 && isColorQuery) {
          ...
          const searchTerm =
            message.match(/(màu\s+sắc|màu).*?([\w\s]+)/i)?.[2] ||
          ...
          if (productInfo.products.length > 0) {
            ...
            productInfo.products.forEach((product) => {
              ...
              userMessage += `\n- ${
                product.name
              }${brandInfo}${colorInfo}: ${product.price.toLocaleString(
                "vi-VN"
            ...
            if (
              productInfo.totalInStock &&
              productInfo.totalInStock > productInfo.products.length
            ) {
    ...
    if (
      message.toLowerCase().includes("ưu đãi") &&
      message.toLowerCase().includes("sinh viên")
    ) {
      if (!response.includes("BACKTOSCHOOL") || !response.includes("10%")) {
        response =
          "Có! GearUp có chương trình ưu đãi giảm 10% cho học sinh và sinh viên khi mua sắm. Bạn cần sử dụng mã 'BACKTOSCHOOL' khi thanh toán và xuất trình thẻ học sinh/sinh viên hợp lệ để được giảm giá.";
      }
    }
