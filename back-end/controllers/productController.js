...
const getProductsByBrand = async (req, res) => {
  try {
    ...
    if (!subCategoryExists) {
      return res
        .status(404)
        .json({ message: "Thương hiệu không tồn tại trong danh mục này" });
    }
    ...
    res
      .status(500)
      .json({
        message: "Lỗi khi lấy sản phẩm theo thương hiệu",
        error: error.message,
      });
  }
  ...
  try {
    ...
    if (brand) {
      if (mongoose.Types.ObjectId.isValid(brand)) {
        ...
        return res
          .status(400)
          .json({ success: false, message: "Brand ID không hợp lệ" });
      }
    }
    if (stockStatus) {
...
const getSimilarProducts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi khi lấy sản phẩm tương tự", error: error.message });
  }
};
