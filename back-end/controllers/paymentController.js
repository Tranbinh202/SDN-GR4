...
exports.createPayment = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Lỗi khi tạo thanh toán qua PayOS" });
  }
};
...
exports.handlePayOSCallback = async (req, res) => {
  try {
    ...
    const order = await Order.findOne({ order_code: orderCode }).populate(
      "items.product_id"
    );
    ...
    if (status === "PAID") {
      try {
        ...
        console.log(
          "📢 Đang tạo thông báo thanh toán cho khách hàng:",
        ...
        const notification = new Notification({
          ...
          message: `Đơn hàng ${
            order.order_code
          } của bạn đã được thanh toán với số tiền ${order.total_price.toLocaleString(
            "vi-VN"
          )} VNĐ qua VietQR.`,
...
exports.fixFailedPaymentOrders = async (req, res) => {
  try {
    ...
    console.log(
      `Found ${orders.length} orders with failed payment but not cancelled`
    );
...
exports.getPaymentLink = async (req, res) => {
  try {
    ...
    if (!order) {
      return res.status(404).json({
        message:
          "Không tìm thấy đơn hàng hoặc đơn hàng không phải thanh toán chuyển khoản",
      });
    }
